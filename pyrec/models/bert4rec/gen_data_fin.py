...
flags.DEFINE_float("train_size", 0.8, "Train dataset size")
flags.DEFINE_bool("use_bst", True, "If set true, we solve relevance prediction task")
...
class TrainingInstance(object):
    ...
    def __init__(self, info, tokens, masked_lm_positions, masked_lm_labels, bst_target=1):
...
def write_instance_to_example_files(instances, max_seq_length, max_predictions_per_seq, vocab, output_files):
    ...
    for (inst_index, instance) in enumerate(instances):
        ...
        features["bst_target"] = create_int_feature([instance.bst_target])
...
def create_training_instances(all_documents_raw,
    ...
    if force_last:
        ...
        for user, item_seq in all_documents_raw.items():
            ...
            # TODO: (@whiteRa2bit, 2020-07-01) Add slide
    ...
    if force_last:
        for user in all_documents:
            instances.extend(create_instances_from_document_test(all_documents, vocab, rng, user, max_seq_length))
        ...
        for step in range(dupe_factor):
            pool.apply_async(
                ...
                args=(all_documents, user, max_seq_length, short_seq_prob, masked_lm_prob, max_predictions_per_seq,
                      vocab, random.Random(random.randint(1, 10000)), mask_prob, step),
                callback=log_result)
        pool.close()
        pool.join()
        ...
        for user in all_documents:
            instances.extend(
                mask_last(all_documents, user, max_seq_length, short_seq_prob, masked_lm_prob, max_predictions_per_seq,
                          vocab, rng))
...
def create_instances_threading(all_documents, user, max_seq_length, short_seq_prob, masked_lm_prob,
    ...
    for user in all_documents:
        cnt += 1
        if cnt % 1000 == 0:
            print("step: {}, name: {}, step: {}, time: {}".format(step,
                                                                  multiprocessing.current_process().name, cnt,
                                                                  time.clock() - start_time))
            start_time = time.clock()
        instances.extend(
            create_instances_from_document_train(all_documents, user, max_seq_length, short_seq_prob, masked_lm_prob,
                                                 max_predictions_per_seq, vocab, rng, mask_prob))
    return instances


def mask_last(all_documents, user, max_seq_length, short_seq_prob, masked_lm_prob, max_predictions_per_seq, vocab, rng):
...
def create_instances_from_document_test(all_documents, vocab, rng, user, max_seq_length):
    """Creates `TrainingInstance`s for a single document."""
    document = all_documents[user]
    max_num_tokens = max_seq_length
    ...
    positive_instance = TrainingInstance(
        info=info,
        tokens=tokens,
    ...
    negative_instance = TrainingInstance(
        ...
        tokens=tokens,
        masked_lm_positions=masked_lm_positions,
        masked_lm_labels=[rng.choice(vocab.vocab_words)],
        bst_target=0
    )
...
def gen_samples(data,
                ...
                pool_size,
                force_last=False):
    def bst_train_test_split():
        random_idxs = np.arange(len(instances))
        rng.shuffle(random_idxs)

        train_idxs = random_idxs[:int(len(instances) * FLAGS.train_size)]
        test_idxs = random_idxs[int(len(instances) * FLAGS.train_size):]

        train_instances = np.array(instances)[train_idxs]
        test_instances = np.array(instances)[test_idxs]
    ...
    if FLAGS.use_bst:
        train_instances, test_instances = bst_train_test_split()
...
def main():
    ...
    if FLAGS.use_bst:
        ...
        output_filename = output_dir + dataset_name + version_id
        gen_samples(
            user_full_data,
            output_filename,
            rng,
    ...
    with open(his_file_name, 'wb') as output_file:
        pickle.dump(user_full_data_output, output_file, protocol=2)
    print('done.')
